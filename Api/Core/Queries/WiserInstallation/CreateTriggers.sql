...
CREATE TRIGGER `DetailInsert` AFTER INSERT ON `wiser_itemdetail` FOR EACH ROW BEGIN
    IF IFNULL(@saveHistory, TRUE) = TRUE THEN
        ...
        VALUES ('UPDATE_ITEM', 'wiser_itemdetail', NEW.id, NEW.item_id, IFNULL(@_username, USER()), NEW.`key`, '', CONCAT_WS('', NEW.`value`, NEW.`long_value`), NEW.language_code, NEW.groupname);
    END IF;
...
CREATE TRIGGER `DetailUpdate` AFTER UPDATE ON `wiser_itemdetail` FOR EACH ROW BEGIN
    ...
    IF oldvalue <> newValue THEN
        IF IFNULL(@saveHistory, TRUE) = TRUE THEN
            INSERT INTO wiser_history (action, tablename, target_id, item_id, changed_by, field, oldvalue, newvalue, language_code, groupname)
            VALUES ('UPDATE_ITEM', 'wiser_itemdetail', NEW.`id`, NEW.`item_id`, IFNULL(@_username, USER()), NEW.`key`, oldValue, newValue, NEW.`language_code`, NEW.`groupname`);
        END IF;
...
CREATE TRIGGER `DetailDelete` AFTER DELETE ON `wiser_itemdetail` FOR EACH ROW BEGIN
    IF IFNULL(@saveHistory, TRUE) = TRUE THEN
        INSERT INTO wiser_history (action, tablename, target_id, item_id, changed_by, field, oldvalue, newvalue, language_code, groupname)
        VALUES ('UPDATE_ITEM', 'wiser_itemdetail', OLD.id, OLD.item_id, IFNULL(@_username, USER()), OLD.`key`, CONCAT_WS('', OLD.`value`, OLD.`long_value`), '', OLD.language_code, OLD.groupname);
    END IF;
...
CREATE TRIGGER `ObjectInsert` AFTER INSERT ON `easy_objects` FOR EACH ROW BEGIN
    IF IFNULL(@saveHistory, TRUE) = TRUE THEN
        INSERT INTO wiser_history (action, tablename, item_id, changed_by, field, oldvalue, newvalue)
        VALUES ('CREATE_OBJECT', 'easy_objects', NEW.id, IFNULL(@_username, USER()), '', '', '');

        IF IFNULL(NEW.`typenr`, '') <> '' THEN
            INSERT INTO wiser_history (action, tablename, item_id, changed_by, field,oldvalue, newvalue)
            VALUES ('UPDATE_OBJECT', 'easy_objects', NEW.`id`, IFNULL(@_username, USER()), 'typenr', NULL, NEW.`typenr`);
        END IF;

        IF IFNULL(NEW.`key`, '') <> '' THEN
            INSERT INTO wiser_history (action, tablename, item_id, changed_by, field,oldvalue, newvalue)
            VALUES ('UPDATE_OBJECT', 'easy_objects', NEW.`id`, IFNULL(@_username, USER()), 'key', NULL, NEW.`key`);
        END IF;

        IF IFNULL(NEW.`value`, '') <> '' THEN
            INSERT INTO wiser_history (action, tablename, item_id, changed_by, field,oldvalue, newvalue)
            VALUES ('UPDATE_OBJECT', 'easy_objects', NEW.`id`, IFNULL(@_username, USER()), 'value', NULL, NEW.`value`);
        END IF;

        IF IFNULL(NEW.`order`, '') <> '' THEN
            INSERT INTO wiser_history (action, tablename, item_id, changed_by, field,oldvalue, newvalue)
            VALUES ('UPDATE_OBJECT', 'easy_objects', NEW.`id`, IFNULL(@_username, USER()), 'order', NULL, NEW.`order`);
        END IF;

        IF IFNULL(NEW.`active`, '') <> '' THEN
            INSERT INTO wiser_history (action, tablename, item_id, changed_by, field,oldvalue, newvalue)
            VALUES ('UPDATE_OBJECT', 'easy_objects', NEW.`id`, IFNULL(@_username, USER()), 'active', NULL, NEW.`active`);
        END IF;

        IF IFNULL(NEW.`description`, '') <> '' THEN
            INSERT INTO wiser_history (action, tablename, item_id, changed_by, field,oldvalue, newvalue)
            VALUES ('UPDATE_OBJECT', 'easy_objects', NEW.`id`, IFNULL(@_username, USER()), 'description', NULL, NEW.`description`);
        END IF;

        IF IFNULL(NEW.`visibleForCustomer`, '') <> '' THEN
            INSERT INTO wiser_history (action, tablename, item_id, changed_by, field,oldvalue, newvalue)
            VALUES ('UPDATE_OBJECT', 'easy_objects', NEW.`id`, IFNULL(@_username, USER()), 'visibleForCustomer', NULL, NEW.`visibleForCustomer`);
        END IF;
    END IF;
END;

DROP TRIGGER IF EXISTS `ObjectUpdate`;
CREATE TRIGGER `ObjectUpdate` AFTER UPDATE ON `easy_objects` FOR EACH ROW BEGIN
    IF IFNULL(@saveHistory, TRUE) = TRUE THEN
        IF IFNULL(NEW.`typenr`, '') <> IFNULL(OLD.`typenr`, '') THEN
            INSERT INTO wiser_history (action, tablename, item_id, changed_by, field,oldvalue, newvalue)
            VALUES ('UPDATE_OBJECT', 'easy_objects', NEW.`id`, IFNULL(@_username, USER()), 'typenr', OLD.`typenr`, NEW.`typenr`);
        END IF;

        IF IFNULL(NEW.`key`, '') <> IFNULL(OLD.`key`, '') THEN
            INSERT INTO wiser_history (action, tablename, item_id, changed_by, field,oldvalue, newvalue)
            VALUES ('UPDATE_OBJECT', 'easy_objects', NEW.`id`, IFNULL(@_username, USER()), 'key', OLD.`key`, NEW.`key`);
        END IF;

        IF IFNULL(NEW.`value`, '') <> IFNULL(OLD.`value`, '') THEN
            INSERT INTO wiser_history (action, tablename, item_id, changed_by, field,oldvalue, newvalue)
            VALUES ('UPDATE_OBJECT', 'easy_objects', NEW.`id`, IFNULL(@_username, USER()), 'value', OLD.`value`, NEW.`value`);
        END IF;

        IF IFNULL(NEW.`order`, '') <> IFNULL(OLD.`order`, '') THEN
            INSERT INTO wiser_history (action, tablename, item_id, changed_by, field,oldvalue, newvalue)
            VALUES ('UPDATE_OBJECT', 'easy_objects', NEW.`id`, IFNULL(@_username, USER()), 'order', OLD.`order`, NEW.`order`);
        END IF;

        IF IFNULL(NEW.`active`, '') <> IFNULL(OLD.`active`, '') THEN
            INSERT INTO wiser_history (action, tablename, item_id, changed_by, field,oldvalue, newvalue)
            VALUES ('UPDATE_OBJECT', 'easy_objects', NEW.`id`, IFNULL(@_username, USER()), 'active', OLD.`active`, NEW.`active`);
        END IF;

        IF IFNULL(NEW.`description`, '') <> IFNULL(OLD.`description`, '') THEN
            INSERT INTO wiser_history (action, tablename, item_id, changed_by, field,oldvalue, newvalue)
            VALUES ('UPDATE_OBJECT', 'easy_objects', NEW.`id`, IFNULL(@_username, USER()), 'description', OLD.`description`, NEW.`description`);
        END IF;

        IF IFNULL(NEW.`visibleForCustomer`, '') <> IFNULL(OLD.`visibleForCustomer`, '') THEN
            INSERT INTO wiser_history (action, tablename, item_id, changed_by, field,oldvalue, newvalue)
            VALUES ('UPDATE_OBJECT', 'easy_objects', NEW.`id`, IFNULL(@_username, USER()), 'visibleForCustomer', OLD.`visibleForCustomer`, NEW.`visibleForCustomer`);
        END IF;
    END IF;
END;

DROP TRIGGER IF EXISTS `ObjectDelete`;
CREATE TRIGGER `ObjectDelete` AFTER DELETE ON `easy_objects` FOR EACH ROW BEGIN
    IF IFNULL(@saveHistory, TRUE) = TRUE THEN
        INSERT INTO wiser_history (action, tablename, item_id, changed_by, field, oldvalue, newvalue)
        VALUES ('DELETE_OBJECT', 'easy_objects', OLD.id, IFNULL(@_username, USER()), 'key', OLD.`key`, '');
    END IF;
END;