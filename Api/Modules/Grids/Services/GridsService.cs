...
namespace Api.Modules.Grids.Services
{
    ...
    public class GridsService : IGridsService, IScopedService
    {
        ...
        /// <inheritdoc />
        public async Task<ServiceResult<GridSettingsAndDataModel>> GetGridDataAsync(int moduleId, ModuleGridDataSettings gridData, ClaimsIdentity identity, bool isForExport = false)
        {
            if (moduleId <= 0)
            {
                throw new ArgumentNullException(nameof(moduleId));
            ...
            // Build the queries.
            var selectQuery = gridData.CustomQuery;
            if (String.IsNullOrWhiteSpace(selectQuery))
            {
                return new ServiceResult<GridSettingsAndDataModel>(results);
            }

            var countQuery = gridData.CountQuery;
            ...
            var userId = IdentityHelpers.GetWiserUserId(identity);
            clientDatabaseConnection.AddParameter("userId", userId);
            (selectQuery, countQuery) = BuildGridQueries(gridData.GridReadOptions, selectQuery, countQuery, identity, null, moduleSettingsModel.FieldMappings);

            // Get the count, but only if this is not the first load.
            if (!results.ClientSidePaging && !String.IsNullOrWhiteSpace(countQuery) && (gridData.GridReadOptions?.FirstLoad ?? true))
            {
                var countDataTable = await clientDatabaseConnection.GetAsync(countQuery);
                if (countDataTable.Rows.Count > 0 && !countDataTable.Rows[0].IsNull(0))
                {
