    ...
    public class GridsService : IGridsService, IScopedService
    {
            ...
            ClaimsIdentity identity)
        {
                ...
                if (gridOptionsDataTable.Columns.Contains("search_count_query"))
                {
                    ...
                    if (String.IsNullOrWhiteSpace(countQuery) && !String.IsNullOrWhiteSpace(selectQuery) && selectQuery.Contains("{limit}", StringComparison.OrdinalIgnoreCase))
                    {
                        countQuery = $@"SELECT COUNT(*) FROM (
                                        {selectQuery.Replace("{limit}", "").Replace("{sort}", "").Trim(';')}
                                    ) AS x";
                    }
                }
            ...
            else if (!String.IsNullOrWhiteSpace(gridOptionsValue))
            {
                try
                {
                    var mainGridOptions = JsonConvert.DeserializeObject<GridSettingsAndDataModel>(gridOptionsValue.Replace("{itemId}", itemId.ToString()));
            ...
            switch (mode)
            {
                ...
                case EntityGridModes.LinkOverview when !String.IsNullOrWhiteSpace(selectQuery):
                {
                    ...
                    selectQuery = selectQuery.Replace("'{itemId}'", "?itemId");
                    countQuery = countQuery?.Replace("'{itemId}'", "?itemId");
                    selectQuery = selectQuery.Replace("{itemId}", "?itemId");
                    countQuery = countQuery?.Replace("{itemId}", "?itemId");
            ...
            string tablePrefix = null)
        {
            ...
            if (options == null)
            {
                selectQuery = selectQuery.Replace("{limit}", "").Replace("{sort}", defaultSort).Replace("{filters}", "") ?? "";
            }
            else
            {
                ...
                int AddFiltersToQuery(int counter, GridFilterModel filter, StringBuilder filtersQuery, (string, List<string>) subWhereClause)
                {
                    ...
                    selectQuery = selectQuery.Replace($"{{{filter.Field.ToMySqlSafeValue(false)}_{filter.Operator.ToMySqlSafeValue(false)}}}", filter.Value.ToMySqlSafeValue(false)) ?? "";
                    countQuery = countQuery.Replace($"{{{filter.Field.ToMySqlSafeValue(false)}_{filter.Operator.ToMySqlSafeValue(false)}}}", filter.Value.ToMySqlSafeValue(false)) ?? "";

                    selectQuery = selectQuery.Replace($"{{{filter.Field.ToMySqlSafeValue(false)}}}", filter.Value.ToMySqlSafeValue(false)) ?? "";
                    countQuery = countQuery.Replace($"{{{filter.Field.ToMySqlSafeValue(false)}}}", filter.Value.ToMySqlSafeValue(false)) ?? "";
                    ...
                    selectQuery = selectQuery.Replace($"{{{fieldName}_filter}}", $" {@operator} {parameterInWhere}") ?? "";
                    countQuery = countQuery.Replace($"{{{fieldName}_filter}}", $" {@operator} {parameterInWhere}") ?? "";
                    selectQuery = selectQuery.Replace($"{{{fieldName}_has_filter}}", "1") ?? "";
                    countQuery = countQuery.Replace($"{{{fieldName}_has_filter}}", "1") ?? "";
                    return counter;
                }
                ...
                else
                {
                    ...
                    selectQuery = selectQuery.Replace("{limit}", "LIMIT ?skip, ?take") ?? "";
                }

                if (options.Sort == null || !options.Sort.Any())
                {
                    selectQuery = selectQuery.Replace("{sort}", defaultSort ?? "") ?? "";
                }
                else
                {
                    ...
                    if (shouldCreateJoinsForSorting)
                    {
                        ...
                        selectQuery = selectQuery.Replace("{joinsForSorting}", joinsForSorting.ToString());
                    }

                    selectQuery = selectQuery.Replace("{sort}", $"ORDER BY {String.Join(", ", options.Sort.Select(s => $"{(!shouldCreateJoinsForSorting || ItemColumns.Any(x => x.Equals(s.Field, StringComparison.OrdinalIgnoreCase)) ? $"`{s.Field.UnmakeJsonPropertyName().ToMySqlSafeValue(false)}`" : $"CONCAT_WS('', `{s.Field.UnmakeJsonPropertyName().ToMySqlSafeValue(false)}`.`value`, `{s.Field.UnmakeJsonPropertyName().ToMySqlSafeValue(false)}`.`long_value`)")} {s.Dir.ToMySqlSafeValue(false).ToUpperInvariant()}"))}") ?? "";
                }

                if (options.Filter?.Filters == null || !options.Filter.Filters.Any())
                {
                    selectQuery = selectQuery.Replace("{filters}", "") ?? "";
                    countQuery = countQuery.Replace("{filters}", "") ?? "";

                    selectQuery = selectQuery.Replace("{hasWhere}", "0") ?? "";
                    countQuery = countQuery.Replace("{hasWhere}", "0") ?? "";

                    selectQuery = selectQuery.Replace("{where}", "TRUE") ?? "";
                    countQuery = countQuery.Replace("{where}", "TRUE") ?? "";
                }
                else
                {
                    ...
                    selectQuery = selectQuery.Replace("{logic}", logic) ?? "";
                    countQuery = countQuery.Replace("{logic}", logic) ?? "";
                    ...
                    selectQuery = selectQuery.Replace("{filters}", filtersQuery.ToString()) ?? "";
                    countQuery = countQuery.Replace("{filters}", filtersQuery.ToString()) ?? "";

                    selectQuery = selectQuery.Replace("{hasWhere}", whereClause.Any() ? "1" : "0") ?? "";
                    countQuery = countQuery.Replace("{hasWhere}", whereClause.Any() ? "1" : "0") ?? "";
                    ...
                    else
                    {
                        ...
                        selectQuery = selectQuery.Replace("{where}", value) ?? "";
                        countQuery = countQuery.Replace("{where}", value) ?? "";
                    }
                }
                ...
                if (options.ExtraValuesForQuery != null)
                {
                    ...
                    {
                        selectQuery = selectQuery.Replace($"{{{pair.Key}}}", pair.Value.ToMySqlSafeValue(false)) ?? "";
                        countQuery = countQuery.Replace($"{{{pair.Key}}}", pair.Value.ToMySqlSafeValue(false)) ?? "";
                    }
                }
            }
        ...
        private async Task<GridSettingsAndDataModel> GridSettingsAndDataModelFromFieldOptionsAsync(int propertyId, string rawOptions, ulong itemId)
        {
            ...
            try
            {
                results = JsonConvert.DeserializeObject<GridSettingsAndDataModel>(rawOptions.Replace("{itemId}", itemId.ToString()));
        ...
        private string DoReplacementsForGridMutationQuery(ClaimsIdentity identity, string encryptedId, Dictionary<string, object> data, string query, ulong itemId)
        {
            ...
            foreach (var key in data.Keys)
            {
                ...
                query = query.Replace($"{{propertyKey{counter}}}", key.UnmakeJsonPropertyName());
                query = query.Replace($"{{propertyValue{counter}}}", data[key] == null ? "" : data[key].ToString().ToMySqlSafeValue(false));
                counter++;
            }

            var userId = IdentityHelpers.GetWiserUserId(identity);
            query = query.Replace("{userId}", userId.ToString());
            query = query.Replace("{username}", IdentityHelpers.GetUserName(identity, true) ?? "");
            query = query.Replace("{userEmailAddress}", IdentityHelpers.GetEmailAddress(identity) ?? "");
            query = query.Replace("{userType}", IdentityHelpers.GetRoles(identity) ?? "");
            query = query.Replace("{encryptedId}", encryptedId);
