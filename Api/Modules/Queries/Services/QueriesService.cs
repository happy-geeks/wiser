...
public class QueriesService : IQueriesService, IScopedService
{
    ...
    private static async Task LockTables(IDatabaseConnection targetDatabase)
    {
        var lockQuery = $"""
                         LOCK TABLES
    ...
    private static async Task UnlockTables(IDatabaseConnection targetDatabase)
    {
        var unlockQuery = "UNLOCK TABLES";
        await targetDatabase.ExecuteAsync(unlockQuery);
    }

    private async Task CreateAsyncOnDataBase(IDatabaseConnection targetDatabase, QueryModel queryModel, ClaimsIdentity identity, string description)
    {
        using var scope = serviceProvider.CreateScope();
        ...
        try
        {
            ...
            var itemInsertQuery = $"""
                                   INSERT INTO {WiserTableNames.WiserQuery}
                                                   (
                                                       id,
                                                       description,
                                                       query,
        ...
        {
            await UnlockTables(targetDatabase);
        }
    }
    ...
    public async Task<ServiceResult<bool>> UpdateAsync(ClaimsIdentity identity, QueryModel queryModel)
    {
        ...
        var query = $"""
                     UPDATE {WiserTableNames.WiserQuery}
        ...
        query = $"""
                 INSERT IGNORE INTO {WiserTableNames.WiserPermission} (role_id, query_id, permissions)
                 VALUES(?roleId, ?id, 15)
                 """;
    ...
    public async Task<ServiceResult<bool>> DeleteAsync(ClaimsIdentity identity, int id)
    {
        ...
        var query = $"""
                     DELETE FROM {WiserTableNames.WiserQuery} WHERE id = ?id;
    ...
    public async Task<ServiceResult<JToken>> GetQueryResultAsJsonAsync(ClaimsIdentity identity, int id, bool asKeyValuePair, List<KeyValuePair<string, object>> parameters)
    {
        ...
        var result = dataTable.Rows.Count == 0 ? [] : dataTable.ToJsonArray(skipNullValues: true);
