...
public class TemplateDataService : ITemplateDataService, IScopedService
{
    ...
    public async Task<Dictionary<int, int>> GetPublishedEnvironmentsAsync(int templateId, TenantModel branch = null)
    {
        ...
        if (branch != null)
        {
            ...
            branchDatabaseConnection.AddParameter("templateId", templateId);
    ...
    /// <inheritdoc />
    public async Task<(int Id, int Version, Environments Environment, bool Removed)> GetLatestVersionAsync(int templateId)
    {
    ...
    /// <inheritdoc />
    public async Task<int> UpdatePublishedEnvironmentAsync(int templateId, int version, Environments environment, PublishLogModel publishLog, string username, TenantModel branch = null)
    {
        using var scope = serviceProvider.CreateScope();
        ...
        var query = $"""
                     UPDATE {WiserTableNames.WiserTemplate}
        ...
        query = $"""
                 INSERT INTO {WiserTableNames.WiserTemplatePublishLog}
                 (
                     template_id,
                     old_live,
                     old_accept,
    ...
    public async Task<List<SearchResultModel>> SearchAsync(string searchValue, string encryptionKey)
    {
        ...
        query = $"""
                 SELECT template.template_id, template.template_data
                 FROM {WiserTableNames.WiserTemplate} AS template
                 LEFT JOIN {WiserTableNames.WiserTemplate} AS otherVersion ON otherVersion.template_id = template.template_id AND otherVersion.version > template.version
    ...
    public async Task<int> CreateAsync(string name, int? parent, TemplateTypes type, string username, string editorValue = null, int? ordering  = null)
    {
        ...
        var dataTable = await clientDatabaseConnection.GetAsync($"""
                                                                 SET @id = (SELECT MAX(template_id)+1 FROM {WiserTableNames.WiserTemplate});
    ...
    public async Task<TemplateSettingsModel> GetParentAsync(int templateId)
    {
        ...
        var query = $"""
                     SELECT parent.template_id, parent.template_name
                     FROM {WiserTableNames.WiserTemplate} AS template
                     LEFT JOIN {WiserTableNames.WiserTemplate} AS otherVersion ON otherVersion.template_id = template.template_id AND otherVersion.version > template.version
    ...
    public async Task<int> GetHighestOrderNumberOfChildrenAsync(int? templateId)
    {
        ...
        var query = $"""
                     SELECT IFNULL(MAX(template.ordering), 0) AS ordering
                     FROM {WiserTableNames.WiserTemplate} AS template
                     LEFT JOIN {WiserTableNames.WiserTemplate} AS otherVersion ON otherVersion.template_id = template.template_id AND otherVersion.version > template.version
    ...
    public async Task<bool> DeleteAsync(int templateId, string username, bool alsoDeleteChildren = true)
    {
        ...
        query = $"""
                 INSERT INTO {WiserTableNames.WiserTemplate} (template_id, parent_id, template_name, template_type, ordering, version, removed, added_on, added_by, changed_on, changed_by, is_dirty)
