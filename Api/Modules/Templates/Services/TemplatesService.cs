...
public class TemplatesService : ITemplatesService, IScopedService
{
    ...
    private static void InitTemplateQueries()
    {
        if (TemplateQueryStrings.Count == 0)
        {
            ...
            TemplateQueryStrings.Add("IMPORTEXPORT_GET_ENTITY_NAMES", """
                                                                      SELECT `name`, module_id AS moduleId
                                                                      FROM wiser_entity
                                                                      WHERE `name` <> ''
                                                                      ORDER BY `name`
                                                                      """);
            TemplateQueryStrings.Add("SET_DATA_SELECTOR_REMOVED", """
                                                                  UPDATE wiser_data_selector
                                                                  SET removed = 1
                                                                  WHERE id = {itemId};

                                                                  SELECT ROW_COUNT() > 0 AS updateSuccessful;
                                                                  """);
            TemplateQueryStrings.Add("UPDATE_LINK", """
                                                    SET @_linkId = {linkId};
                                                    ...
                                                    WHERE id = @_linkId;
                                                    """);
            TemplateQueryStrings.Add("GET_OPTIONS_FOR_DEPENDENCY", """
                                                                   SELECT DISTINCT entity_name AS entityName, IF(tab_name = '', 'Gegevens', tab_name) as tabName, CONCAT(IF(tab_name = '', 'Gegevens', tab_name), ' --> ', display_name) AS displayName, property_name AS propertyName FROM wiser_entityproperty
            ...
            TemplateQueryStrings.Add("GET_ENTITY_PROPERTIES_ADMIN", """
                                                                    SELECT id, entity_name AS entityName, tab_name AS tabName, display_name AS displayName, ordering FROM wiser_entityproperty
                                                                    WHERE tab_name = IF('{tabName}' = 'Gegevens', '', '{tabName}') AND entity_name = '{entityName}'
                                                                    ORDER BY ordering ASC
                                                                    """);
            TemplateQueryStrings.Add("GET_ENTITY_LIST", """
                                                        SELECT 
                                                        	entity.id,
                                                        ...
                                                        """);
            TemplateQueryStrings.Add("GET_LANGUAGE_CODES", """
                                                           SELECT
                                                           ...
                                                           """);
            TemplateQueryStrings.Add("GET_ENTITY_PROPERTIES_TABNAMES", """
                                                                       SELECT id, IF(tab_name = '', 'Gegevens', tab_name) AS tabName FROM wiser_entityproperty
                                                                       WHERE entity_name = '{entityName}'
                                                                       GROUP BY tab_name
                                                                       ORDER BY tab_name ASC
                                                                       """);
            TemplateQueryStrings.Add("GET_ROLES", """
                                                  SELECT id AS id, role_name AS roleName FROM wiser_roles
                                                  WHERE role_name != ''
            ...
            TemplateQueryStrings.Add("UPDATE_ENTITY_PROPERTY_PERMISSIONS", """
                                                                           INSERT INTO `wiser_permission` (
                                                                               role_id, 
                                                                               entity_name, 
                                                                               item_id, 
                                                                               entity_property_id, 
                                                                           ...
                                                                           """);
            TemplateQueryStrings.Add("GET_GROUPNAME_FOR_SELECTION", """
                                                                    SELECT DISTINCT group_name AS groupName FROM `wiser_entityproperty`
            ...
            TemplateQueryStrings.Add("INSERT_ENTITYPROPERTY", """
                                                              SET @newOrderNr = IFNULL((SELECT MAX(ordering)+1 FROM wiser_entityproperty WHERE entity_name='{entityName}' AND tab_name = '{tabName}'),1);
                                                              ...
                                                              """);
            TemplateQueryStrings.Add("RENAME_ITEM", """
                                                    SET @item_id={itemid:decrypt(true)};
                                                    SET @newname='{name}';

                                                    UPDATE wiser_item SET title=@newname WHERE id=@item_id LIMIT 1;
                                                    """);
            TemplateQueryStrings.Add("GET_UNDERLYING_LINKED_TYPES", """
                                                                    SET @_entity_name = IF(
                                                                        '{entityName}' NOT LIKE '{%}',
                                                                        '{entityName}',
                                                                    ...
                                                                    ORDER BY entityType
                                                                    """);
            TemplateQueryStrings.Add("GET_PARENT_LINKED_TYPES", """
                                                                SET @_entity_name = IF(
                                                                    '{entityName}' NOT LIKE '{%}',
                                                                    '{entityName}',
                                                                    # Check for old query string name. Takes the first item in a comma-separated list of entity type names.
                                                                ...
                                                                ORDER BY entityType
                                                                """);
            TemplateQueryStrings.Add("GET_ITEM_VALUE", """
                                                       SELECT
                                                       	id,
                                                           `key`,
                                                       ...
                                                       """);
            TemplateQueryStrings.Add("GET_ITEM_DETAILS", """
                                                         SET @_itemId = {itemId:decrypt(true)};
                                                         ...
                                                         GROUP BY i.id, p.id
                                                         """);
            TemplateQueryStrings.Add("GET_TITLE", """
                                                  SET @_itemId = {itemId:decrypt(true)};
                                                  SELECT title FROM wiser_item WHERE id = @_itemId;
                                                  """);
            TemplateQueryStrings.Add("GET_PROPERTIES_OF_ENTITY", """
                                                                 SELECT
                                                                 ...
                                                                 """);
            TemplateQueryStrings.Add("IMPORTEXPORT_GET_LINK_TYPES", """
                                                                    SELECT type AS id, `name`
                                                                    FROM wiser_link
                                                                    ORDER BY `name`
                                                                    """);
            TemplateQueryStrings.Add("MOVE_ITEM", """
                                                  #Item verplaatsen naar ander item
            ...
            TemplateQueryStrings.Add("GET_COLUMNS_FOR_LINK_TABLE", """
                                                                   SET @destinationId = {id:decrypt(true)};
                                                                   ...
                                                                   ORDER BY p.ordering;
                                                                   """);
            TemplateQueryStrings.Add("GET_MODULES", """
                                                    SELECT id, name as moduleName
                                                    FROM wiser_module
                                                    ORDER BY name ASC;

                                                    """);
            TemplateQueryStrings.Add("GET_ROLE_RIGHTS", """
                                                        SELECT
            ...
            TemplateQueryStrings.Add("GET_DATA_SELECTOR_BY_ID", """
                                                                SET @_id = {id};
            ...
            TemplateQueryStrings.Add("GET_ITEM_ENVIRONMENTS", """
                                                              SELECT
                                                              ...
                                                              """);
            TemplateQueryStrings.Add("GET_ALL_ITEMS_OF_TYPE", """
                                                              #Get all the items for the treeview
                                                              SET @mid = {moduleid};
                                                              ...
                                                              """);
            TemplateQueryStrings.Add("GET_COLUMNS_FOR_TABLE", """
                                                              SET @selected_id = {itemId:decrypt(true)}; # 3077
                                                              ...
                                                              ORDER BY p.ordering;
                                                              """);
            TemplateQueryStrings.Add("GET_DATA_FOR_TABLE", """
                                                           SET @selected_id = {itemId:decrypt(true)}; # 3077
                                                           SET @userId = {encryptedUserId:decrypt(true)};
                                                           ...
                                                           """);
            TemplateQueryStrings.Add("GET_ITEMLINKS_BY_ENTITY", """
                                                                SELECT
                                                                ...
                                                                GROUP BY type
                                                                """);
            TemplateQueryStrings.Add("GET_ENTITY_PROPERTIES", """
                                                              SET @_entity_name = IF(
                                                                  '{entityName}' NOT LIKE '{%}',
                                                                  '{entityName}',
                                                              ...
                                                              """);
            TemplateQueryStrings.Add("GET_ENTITY_LINK_PROPERTIES", """
                                                                   SET @_link_type = IF(
                                                                       '{link_type}' NOT LIKE '{%}',
                                                                   ...
                                                                   ORDER BY property.dynamicField, property.sort
                                                                   """);
            TemplateQueryStrings.Add("GET_LINKED_TO_ITEMS", """
                                                            #Get all the items for the treeview
                                                            ...
                                                            """);
            TemplateQueryStrings.Add("GET_DATA_FOR_FIELD_TABLE", """
                                                                 SET @selected_id = {itemId:decrypt(true)};
            ...
            TemplateQueryStrings.Add("GET_WISER_LINK_LIST", """
                                                            SELECT *,
    ...
    public async Task<ServiceResult<bool>> SaveAsync(ClaimsIdentity identity, TemplateSettingsModel template, bool skipCompilation = false)
    {
        ...
        var allLinkedTemplates = new List<int>(jsLinks ?? []);
        if (scssLinks != null)
        {
            ...
            foreach (DataRow dataRow in dataTable.Rows)
            {
                ...
                foreach (DataRow dynamicContentRow in dynamicContentDataTable.Rows)
                {
                    ...
                    await clientDatabaseConnection.ExecuteAsync($"""
                                                                 INSERT IGNORE INTO {WiserTableNames.WiserTemplateDynamicContent} (content_id, destination_template_id, added_on, added_by)
    ...
    private DynamicContentOverviewModel LinkedDynamicContentToDynamicContentOverview(LinkedDynamicContentDao linkedContent)
    {
        var overview = new DynamicContentOverviewModel
        {
            ...
            Usages = String.IsNullOrEmpty(linkedContent.Usages) ? [] : linkedContent.Usages.Split(",").ToList()
        };
